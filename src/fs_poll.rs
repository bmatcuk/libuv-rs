...
use uv::{uv_fs_poll_t, uv_fs_poll_init, uv_fs_poll_start, uv_fs_poll_stop, uv_fs_poll_getpath};

/// Additional data stored on the handle
#[derive(Default)]
pub(crate) struct FsPollDataFields {
    fs_poll_cb: Option<Box<dyn FnMut(FsPollHandle, i32, Stat, Stat)>>,
}
...
extern "C" fn uv_fs_poll_cb(
    ...
    if !dataptr.is_null() {
        unsafe {
            if let crate::FsPollData(d) = &mut (*dataptr).addl {
...
impl FsPollHandle {
    ...
    pub fn new(r#loop: &crate::Loop) -> crate::Result<FsPollHandle> {
        ...
        crate::Handle::initialize_data(uv_handle!(handle), crate::FsPollData(Default::default()));
    ...
    pub fn start(&mut self, path: &str, interval: u32, cb: Option<impl FnMut(FsPollHandle, i32, Stat, Stat) + 'static>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if !dataptr.is_null() {
            if let crate::FsPollData(d) = unsafe { &mut (*dataptr).addl } {
                d.fs_poll_cb = cb;
            }
        }

        crate::uvret(unsafe { uv_fs_poll_start(self.handle, uv_cb, path.as_ptr(), interval as _) }).map_err(|e| Box::new(e) as _)
    }
    ...
    pub fn getpath(&self) -> Result<String, Box<dyn std::error::Error>> {
