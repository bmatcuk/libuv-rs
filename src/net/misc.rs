...
use std::ffi::CStr;
use uv::{
    uv_if_indextoiid, uv_if_indextoname, uv_interface_addresses,
    uv_ip4_addr, uv_ip4_name, uv_ip6_addr, uv_ip6_name, UV_IF_NAMESIZE,
};
...
pub fn if_indextoname(ifindex: u32) -> crate::Result<String> {
    ...
    crate::uvret(unsafe {
        uv_if_indextoname(ifindex, buf.as_mut_ptr() as _, &mut size as _)
    })
    .map(|_| {
...
pub fn if_indexto_iid(ifindex: u32) -> crate::Result<String> {
    ...
    let mut buf: Vec<std::os::raw::c_uchar> = Vec::with_capacity(size);
    crate::uvret(unsafe {
        uv_if_indextoiid(ifindex, buf.as_mut_ptr() as _, &mut size as _)
    })
    .map(|_| {
        // size is the length of the string, *not* including the null
        unsafe { buf.set_len(size + 1) };
