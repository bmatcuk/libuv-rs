...
/^pub const uv_fs_type_UV_/ && !/UNKNOWN/ {
...
END {
  ...
  for (i = 1; i <= ntypes; i++)
    print indent indent indent "uv::uv_fs_type_UV_" types[i] " => FsType::" types[i] ",";
  ...
  for (i = 1; i <= ntypes; i++)
    print indent indent indent "FsType::" types[i] " => uv::uv_fs_type_UV_" types[i] ",";
